...
int babyMaker::looper(TChain* chain, char* output_name, int nEvents){
  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) { 
    ...
    for(unsigned int evt = 0; evt < nEventsTree; evt++){
      ...
      //Determine number of FOs
      nFOs_SS = 0;
      for(size_t j = 0; j < tas::mus_p4().size(); j++){
        if(muonID(j, SS_fo_v3) && tas::mus_p4().at(j).pt() > 10) nFOs_SS++;
      }
      for(size_t j = 0; j < tas::els_p4().size(); j++){
        if(electronID(j, SS_fo_v3) && tas::els_p4().at(j).pt() > 10) nFOs_SS++;
      }

      //Muon Tag and Probe -- i is the probe, j is the tag
      bool used = false;
      TRandom r;
      float rndm = r.Rndm();
      for(unsigned int i = 0; i < tas::mus_p4().size(); i++){

        //Require that the probe has pt of 10
        ...
        for(unsigned int j = 0; j < tas::mus_p4().size(); j++){
          ...
          if (!muonID(j,SS_tight_v3)) continue; 
          foundTag = true; 
        ...
        //If probe doesn't pass ID AND there is no tag, veto (why?)
        if(muonID(i, SS_veto_noiso_v3) == 0 && foundTag==false) continue;

        //p4 is the tag of the probe
        p4 = tas::mus_p4().at(i); 

        //Calculate dilep mass, etc.  
        if (foundTag){
          dilep_p4 = p4 + tag_p4;
        ...
        //Fill tree one entry for every probe
        BabyTree->Fill(); 
        InitLeptonBranches(); 

      }

      //Electron Tag and Probe -- i is the probe, j is the tag
      for(unsigned int i=0; i < tas::els_p4().size(); i++){
        ...
        for(unsigned int j = 0; j < tas::els_p4().size(); j++){
          ...
          if (!electronID(j,SS_medium_v3)) continue;
          foundTag = true; 
        ...
        if(electronID(i, SS_veto_noiso_v3)==0 && !foundTag) continue;
